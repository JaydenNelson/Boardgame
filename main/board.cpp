...
//---------------------------------------------------------------------------------------------------------------
void setObst(cell* row, const int& z, const char& m) {
    for (int i = 0;i < z;i++) {
        row[i].setMarble(m);
    }
    if (z == 9) {                      // increased row size to 9 
        char pos[6] = { 0,1,2,6,7,8 }; // row positions to leave as + 
        for (int i = 0;i < 6;i++) {    
            row[pos[i]].setMarble('+');
        }
    }
...
board::board() :woc(0), boc(0) {
    ...
    for (int i = 0;i < 9;i++) {
        ...
        if (i <= 3) {
            ...
            //cout << " if statement 100 sucessful" << endl;
            ...
            //cout << " if statement 114 sucessful" << endl;
        }
          
    //------------------------------------------------------------------------------------------
        else if (i == 4) {

           // cout << " if statement 118 sucessful" << endl;
    ...
    if (!cell::validateLocation(l))//confirm existence of location
      ...
      //      if (marble != m || marble == '#')                 -------------------------------------------------------------------------------------------------------------
        ...
        for (int i = 1;i < n;i++) {
            ...
            if (cp == nullptr || marble != cp->getMarble()|| marble=='#')//confirm availability and type of claimed resources ------------------------------------- added || marble=='#'
                return false;
        }
